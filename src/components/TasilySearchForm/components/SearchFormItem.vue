<template>
	...
	<component
		:is="column.search?.render ?? `el-${column.type}`"
		v-bind="{ ...handleSearchProps, ...placeholder, clearable, searchParam: _searchParam }"
		v-model.trim="_searchParam[column?.field]"
	>
...
interface SearchFormItem {
	column: Table.ColumProps;
	searchParam: Record<string, any> | undefined;
}
...
const fieldNames = computed(() => {
	return {
		...
		value: props.column?.field ?? "value",
		children: props.column?.children ?? "children"
	};
});
...
const placeholder = computed(() => {
	const searchColumns = props.column;
	if (["datetimerange", "daterange", "monthrange"].includes(searchColumns?.type ?? "")) {
		return { rangeSeparator: "至", startPlaceholder: "开始时间", endPlaceholder: "结束时间" };
	}
	const placeholder = searchColumns?.type?.includes("input") ? "请输入" : "请选择";
	return { placeholder };
});
